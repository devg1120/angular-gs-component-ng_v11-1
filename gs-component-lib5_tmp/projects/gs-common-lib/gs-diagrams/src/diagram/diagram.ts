...
//gs import { HistoryEntry, History } from './diagram/history';
//gs import { UndoRedo } from './objects/undo-redo';
...
export class Diagram extends Component<HTMLElement> implements INotifyPropertyChanged {
 ...
 //    historyManager: History;
    ...
    /** @ */
     ...
     diagramLayerDiv: HTMLElement;
     adornerLayer: SVGElement;
     eventHandler: DiagramEventHandler;
    /** @ */
    ...
    /** @ */
     nameTable: {};
    /** @ */
     canEnableBlazorObject: boolean = false;
    /** @ */
     pathTable: {};
    /** @ */
     connectorTable: {} = {};
    ...
    /** @ */
     blazorActions: BlazorAction = BlazorAction.Default;
    /** @ */
     commands: {};
    /** @ */
     activeLabel: ActiveLabel = { id: '', parentId: '', isGroup: false, text: undefined };
    /** @ */
    ...
    /** @ */
     isTriggerEvent: Boolean = false;
    /** @ */
     preventNodesUpdate: Boolean = false;
    /** @ */
     preventConnectorsUpdate: Boolean = false;
    /** @ */
     callBlazorModel: Boolean = true;
    /** @ */
     selectionConnectorsList: ConnectorModel[] = [];
    /** @ */
     deleteVirtualObject: boolean = false;
    /** @ */
     realActions: RealAction;
    /** @ */
     previousSelectedObject: (NodeModel | ConnectorModel)[];
     canLayout: boolean = true;
     changedConnectorCollection: ConnectorModel[] = [];
     changedNodesCollection: NodeModel[] = [];
     previousNodeCollection: NodeModel[] = [];
     previousConnectorCollection: ConnectorModel[] = [];
     crudDeleteNodes: Object[] = [];

    // Group update to server when BlazorAction is isGroupAction;
     blazorAddorRemoveCollection: (NodeModel | ConnectorModel)[] = [];
     blazorRemoveIndexCollection: number[] = [];

     diagramid: number = 88123;
    /** @ */
     selectedObject: { helperObject: NodeModel, actualObject: NodeModel } = { helperObject: undefined, actualObject: undefined };
    /**
    ...
    constructor(options?: DiagramModel, element?: HTMLElement | string) {
     ...
     updateAnnotationText(annotations?: PathAnnotationModel[] | ShapeAnnotationModel[]): void {
     ...
     callFromServer(arg: object): void {
     ...
     clearCollection(isConnector?: boolean): void {
    ...
    /* tslint:enable */
     updateSnapSettings(newProp: DiagramModel): void {
     ...
     updateRulerSettings(newProp: DiagramModel): void {
    ...
    protected preRender(): void {
     ...
     initializePrivateVariables(): void {
        if (this.element.id === '') {
    ...
    //  measureNode(node: NodeModel): void {
    ...
    /* tslint:enable */
     removeUserHandlesTemplate(): void {
     ...
     *  @
     */
     addDiagramLayer(layer: LayerModel, layerObject?: Object[]): void {
     ...
     *  @
     */
     removeDiagramLayer(layerId: string): void {
     ...
     */
     ...
     nudgeCommand(direction: NudgeDirection, x?: number, y?: number): void {
     ...
     */
     ...
     insertBlazorDiagramObjects(actualObject: NodeModel | ConnectorModel | SelectorModel): void {
     ...
     */
     ...
     disableStackContainerPadding(wrapper: Container, disable: boolean): void {
    ...
    /* //gs
     ...
     checkCurrentSymbol(currentSymbol: Node | Connector, entry: HistoryEntry): boolean {
    ...
    */
    ...
    /* //gs
     historyChangeTrigger(entry: HistoryEntry, action: HistoryChangeAction): void {
        ...
        var source: (NodeModel | ConnectorModel)[] = [];
        if (entry.category === 'Internal') {
            ...
            var arg: IHistoryChangeArgs | IBlazorHistoryChangeArgs;
            arg = {
   ...
   */
     ...
     */
     ...
     updateBlazorDiagramProperties(attribute: string[], canCall?: boolean): void {
        if (isBlazor() && !canCall) {
            //Need to send the client changes into server side for  APIs changes.
     ...
     * @
     */
     async triggerEvent(eventName: DiagramEvent, args: Object): Promise<void | object> {
        if (args) {
     ...
     updateEventValue(args: IDropEventArgs): void {
    ...
    */
    ...
    /* eslint-disable */
    /** @ */
     UpdateBlazorDiagramModelCollection(
    ...
    // eslint-disable-next-line max-len
     UpdateBlazorLabelOrPortObjects(
     ...
     */
     ...
     removeNodeEdges(elementId: string, id: string, isOutEdges: boolean): void {
    ...
    //Splice the InEdge and OutEdge of the for the node with respect to corresponding connectors that is deleting
     spliceConnectorEdges(connector: ConnectorModel, isSource: boolean): void {
        //let node: Node;
        let edges: string[] = [];
     ...
     */
     ...
     clearObjects(collection?: (NodeModel | ConnectorModel)[]): void {
        let objects: (NodeModel | ConnectorModel)[] = [];
        if (!collection) {
     ...
     */
     ...
     updateConnectorfixedUserHandles(connector: Connector): void {
    ...
    /* tslint:enable */
     updateNodeExpand(node: Node, visibility: boolean): void {
     ...
     updateConnectorAnnotation(connector: Connector): void {
     ...
     removeChildrenFromLayout(nodes: INode[]): INode[] {
     ...
     */
     ...
     renderHiddenUserHandleTemplateLayer(bounds: ClientRect): void {
        //let element: HTMLElement;
     ...
     renderBackgroundLayer(bounds: ClientRect, commonStyle: string): void {
     ...
     renderGridLayer(bounds: ClientRect, commonStyle: string): void {
     ...
     renderDiagramLayer(bounds: ClientRect, commonStyle: string): void {
        const attributes: Object = {
     ...
     renderAdornerLayer(bounds: ClientRect, commonStyle: string): void {
     ...
     renderPortsExpandLayer(bounds: ClientRect, commonStyle: string): void {
     ...
     renderHTMLLayer(bounds: ClientRect, commonStyle: string): void {
     ...
     renderNativeLayer(bounds: ClientRect, commonStyle: string): void {
     ...
     */
     ...
     initObjects(isLoad?: boolean): void {
     ...
     */
     ...
     alignGroup(parents: string[], tempTabel: {}): string[] {
        let newList: string[] = [];
     ...
     addToLayer(obj: NodeModel | ConnectorModel, hasLayers: boolean): void {
     ...
     updateLayer(newProp: DiagramModel): void {
     ...
     updateScrollSettings(newProp: DiagramModel): void {
     ...
     generateData(dataSource: DataSourceModel | DataManager, isNode: boolean): (NodeModel | ConnectorModel)[] {
     ...
     makeData(row: object[], isNode: boolean): Node | Connector {
        let i: number;
     ...
     initNodes(obj: Node, layer: LayerModel): void {
        this.preventDiagramUpdate = true;
        this.initObject(obj as Node, layer);
        this.preventDiagramUpdate = false;
    }

     initConnectors(obj: Connector, layer: LayerModel, independentObj?: boolean): void {
        this.preventDiagramUpdate = true;
     ...
     setZIndex(layer: LayerModel, obj: NodeModel | ConnectorModel): void {
        //should be changed
     ...
     */
     ...
     initObjectExtend(obj: IElement, layer?: LayerModel, independentObj?: boolean): void {
        if (independentObj) {
     ...
     * @
     */
     initObject(obj: IElement, layer?: LayerModel, independentObj: boolean = true, group?: boolean): void {
        if (obj !== undefined) {
            if (independentObj) {
    ...
    /* tslint:enable */
     ...
     getConnectedPort(node: NodeModel, connector: ConnectorModel, isSource?: boolean): PointPortModel {
        if (node && node.ports) {
     ...
     scaleObject(obj: NodeModel, size: number, isWidth: boolean): void {
     ...
     updateDefaultLayoutIcons(node: NodeModel): void {
        if (this.layout.type === 'OrganizationalChart' || this.layout.type === 'HierarchicalTree' ||
     ...
     updateDefaultLayoutIcon(node: NodeModel, icon: IconShapeModel): void {
        if (icon.shape !== 'None') {
    ...
    /* eslint-disable */
     initNode(obj: Node, diagramId: string, group?: boolean): void {
     ...
     */
     ...
     onLoadImageSize(id: string, size: Size): void {
     ...
     updateChildPosition(obj: NodeModel): void {
        for (let i: number = 0; i < obj.children.length; i++) {
     ...
     updateStackProperty(obj: Node, child: Node, index?: number): void {
     ...
     overrideCommands(newCommand: CommandModel, commands: {}): void {
     ...
     initCommandManager(newCommands: CommandModel[], commands: {}): void {
        let i: number = 0;
        if (newCommands) {
     ...
     * @
     */
     updateIconVisibility(node: Node, visibility: boolean): void {
        for (let i: number = 0; i < node.wrapper.children.length; i++) {
     ...
     */
     ...
     updateCanupdateStyle(element: DiagramElement[], value: boolean): void {
     ...
     getZindexPosition(obj: (NodeModel | ConnectorModel), viewId: string): number {
        let objects: (NodeModel | ConnectorModel)[] = [];
     ...
     */
     ...
     renderBasicElement(view: View): void {
     ...
     refreshElements(view: View): void {
        if (!this.isDestroyed) {
     ...
     */
     ...
     renderNodes(overview: View): void {
        if (overview) {
    ...
    getWrapper(nodes: Container, id: string): DiagramElement {
        ...
        id = nodes.id + '_' + id;
        //gs const container: Container = nodes instanceof Canvas ? nodes : this.getPortContainer(this.nameTable[nodes.id]);
        const container: Container = nodes instanceof Canvas ? nodes : this.getPortContainer(this.nameTable[id]);
     ...
     */
     ...
     containsMargin(node: DiagramElement): Boolean {
     ...
     * @
     */
     getIndex(node: NodeModel | ConnectorModel, id: string) {
        //let index: number;
    ...
    /* tslint:enable */
     ...
     getBlazorTextEditArgs(args: IBlazorTextEditEventArgs | ITextEditEventArgs): IBlazorTextEditEventArgs {
     ...
     */
     ...
     modelChanged(newProp: DiagramModel, oldProp: DiagramModel): boolean {
     ...
     resetDiagramActions(action?: DiagramAction): void {
     ...
     */
     ...
     nodePropertyChangeExtend(actualObject: Node, oldObject: Node, node: Node, update: boolean): boolean {
    ...
    /* tslint:disable */
     swimLaneNodePropertyChange(actualObject: Node, oldObject: Node, node: Node, update: boolean): boolean {
    ...
    // eslint-disable-next-line max-len
     ...
     updatePorts(actualObject: Node, flip: FlipDirection): void {
     ...
     updateFlipOffset(element: Container, diffX: number, diffY: number, flip: FlipDirection): void {
     ...
     updateUMLActivity(changedProp: Node, oldObject: Node, actualObject: Node, diagram: Diagram): void {
     ...
     */
     ...
     triggerPropertyChange(propertyChange: boolean, actualObject: Connector, oldProp: Connector, newProp: Connector): void {
        if (!propertyChange) {
     ...
     findInOutConnectPorts(node: NodeModel, isInconnect: boolean): PointPortModel {
     ...
     getPoints(actualObject: Connector, points?: PointModel[]): PointModel[] {
        //let pts: PointModel[];
     ...
     * @
     */
     updateDiagramContainerVisibility(element: DiagramElement, visible: boolean): void {
        if (element instanceof Container) {
            for (let i: number = 0; i < element.children.length; i++) {
     ...
     */
     ...
     updateAnnotations(newProp: Connector, actualObject: Connector): void {
     ...
     updatefixedUserHandle(newProp: Connector, actualObject: Connector): void {
     ...
     * @
     */
     updateConnectorfixedUserHandle(
     ...
     */
     ...
     getPortContainer(actualObject: Node): Container {
        if (actualObject.children) {
     ...
     updateTooltip(actualObject: NodeModel, node: NodeModel): void {
     ...
     */
     ...
     removePreviewChildren(preview: Node): void {
    ...
    /* tslint:disable */
     ...
     getBlazorDragLeaveEventArgs(args: IBlazorDragLeaveEventArgs | IDragLeaveEventArgs): IBlazorDragLeaveEventArgs {
        args = {
     ...
     getDropEventArgs(arg: IBlazorDropEventArgs) {
     ...
     removeChildInNodes(node: NodeModel) {
        if (node) {
            if (node.children) {
     ...
     getBlazorDragEventArgs(args: IBlazorDragEnterEventArgs | IDragEnterEventArgs): IBlazorDragEnterEventArgs {
        args = {
            // eslint-disable-next-line max-len
     ...
     findChild(node: NodeModel, childTable: {}): void {
        let group: NodeModel;
        let newNode: Node;
        for (let i: number = 0; i < node.children.length; i++) {
     ...
     getChildren(node: NodeModel, entryTable: {}, childTable: {}): {} {
        let temp: NodeModel;
        for (let i: number = 0; i < node.children.length; i++) {
     ...
     addChildNodes(node: NodeModel): void {
        let temp: NodeModel;
        for (let i: number = 0; i < node.children.length; i++) {
     ...
     moveNode(node: NodeModel | ConnectorModel): void {
     ...
     */
     ...
     crudOperation(node: Node | Connector, crud: string, getNodesCollection: IDataSource) {
        if (node) {
     ...
     processCrudCollection(newObjects: IDataSource, nodeCrudAction: string, connectorCrudAction: string): void {
        if (newObjects.nodes) {
            const data: Object[] = [];
            let i: number;
     ...
     parameterMap(object: Node | Connector, isNode: boolean): object {
     ...
     getNewUpdateNodes(status: string): IDataSource {
    ...
    /*
     raiseAjaxPost(value: string, url: string): void {
    ...
    */
     getHiddenItems(args: BeforeOpenCloseMenuEventArgs): string[] {
